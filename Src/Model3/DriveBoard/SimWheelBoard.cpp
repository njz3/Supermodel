...
#include "SimWheelBoard.h"
#include <cstdio>
#include <cmath>
#include <algorithm>

inline bool CSimWheelBoard::IsGame(const char* gameName)
{
    return (m_gameInfo.name == gameName) || (m_gameInfo.parent == gameName);
}

void CSimWheelBoard::GetGame(const Game& gameInfo)
{
    m_gameInfo = gameInfo;
}

Game::DriveBoardType CSimWheelBoard::GetType(void)
{
  return Game::DRIVE_BOARD_WHEEL;
}

void CSimWheelBoard::Get7SegDisplays(UINT8 &seg1Digit1, UINT8 &seg1Digit2, UINT8 &seg2Digit1, UINT8 &seg2Digit2)
{
...
void CSimWheelBoard::SaveState(CBlockFile *SaveState)
{
  CDriveBoard::SaveState(SaveState);

  SaveState->NewBlock("SimWheelBoard", __FILE__);
...
void CSimWheelBoard::LoadState(CBlockFile *SaveState)
{
  if (SaveState->FindBlock("SimWheelBoard") != Result::OKAY)
  {
...
void CSimWheelBoard::LoadLegacyState(CBlockFile *SaveState)
{
...
void CSimWheelBoard::Disable(void)
{
  SendStopAll();
  CDriveBoard::Disable();
}

void CSimWheelBoard::Reset(void)
{
  CDriveBoard::Reset();
  ...
  m_simulated = true;  //TODO: make this run-time configurable when simulation mode is supported
...
// 8bits parallel communication from driveboard
UINT8 CSimWheelBoard::Read(void)
{
  if (IsDisabled())
  {
    return 0xFF;
...
// 8bits parallel communication to driveboard
void CSimWheelBoard::Write(UINT8 data)
{
  if (IsDisabled())
  {
    return;
...
UINT8 CSimWheelBoard::TxScud(void)
{
    UINT8 tx = 0;
    if (m_initialized)
    {
        ...
        case 0x2:
            tx = m_dip2; break;                           // DIP switch 2 value
        case 0x3:
    ...
    else
    {
        ...
        {
        case 0:  tx = 0xCF; break;  // Initiate start
        case 1:  tx = 0xCE; break;
        case 2:  tx = 0xCD; break;
        case 3:  tx = 0xCC; break; // Centering wheel
        default:
            m_initialized = true;
            tx = 0x80;
            break;
...
UINT8 CSimWheelBoard::TxSRally2(void)
{
    UINT8 tx = 0;
    if (m_initialized)
    {
...
UINT8 CSimWheelBoard::TxLeMans(void)
{
    UINT8 tx = 0;
    if (m_initialized)
    {
...
UINT8 CSimWheelBoard::SimulateRead(void)
{  
    UINT8 tx = 0;
    switch (m_gameType) {
    case DriveBoardType::Model2C:
        tx = TxLeMans();
        break;
...
void CSimWheelBoard::RxCmdScud(UINT8 cmd)
{
...
void CSimWheelBoard::RxCmdSRally2(UINT8 cmd)
{
    // Following are commands for Scud Race.  Daytona 2 has a compatible command set while Sega Rally 2 is completely different
  // TODO - finish for Scud Race and Daytona 2
  // TODO - implement for Sega Rally 2
...
void CSimWheelBoard::RxCmdLeMans(UINT8 cmd)
{

}
static UINT8 lastCmd = 0;
void CSimWheelBoard::SimulateWrite(UINT8 cmd)
{
    if (lastCmd != cmd) {
        printf("Db.Rx: %02X\n", cmd);
        lastCmd = cmd;
...
void CSimWheelBoard::RunFrame(void)
{
  if (IsDisabled())
  {
    return;
...
void CSimWheelBoard::SimulateFrame(void)
{
  if (!m_initialized)
    m_initState++;
...
UINT8 CSimWheelBoard::IORead8(UINT32 portNum)
{
  UINT8 adcVal;
...
void CSimWheelBoard::IOWrite8(UINT32 portNum, UINT8 data)
{
  switch (portNum)
  {
...
void CSimWheelBoard::SendStopAll(void)
{
...
void CSimWheelBoard::SendSelfCenter(UINT8 val)
{
...
void CSimWheelBoard::SendFriction(UINT8 val)
{
...
void CSimWheelBoard::SendVibrate(UINT8 val)
{
...
uint8_t CSimWheelBoard::ReadADCChannel1()
{
  return 0x00;
}

uint8_t CSimWheelBoard::ReadADCChannel2()
{
  if (m_initialized)
...
uint8_t CSimWheelBoard::ReadADCChannel3()
{
  return 0x80;
}

uint8_t CSimWheelBoard::ReadADCChannel4()
{
  return 0x00;
}
  ...
  : CDriveBoard(config)
{
  ...
  DebugLog("Built Sim Drive Board (wheel)\n");
}

CSimWheelBoard::~CSimWheelBoard(void)
{
